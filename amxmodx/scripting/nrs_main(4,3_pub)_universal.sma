...
new maxentities
new bool:b_Mode
stock spr_bomb, cache_laserbeam
...
new  bool:g_roundstarted, bool:g_roundended, bool:g_gamestarted, g_maxplayers, g_hamnpc, g_fwd_reset, g_fwd_npc_damage_start, g_fwd_npc_damage_delayed, g_fwd_death, g_fwd_hook_catched, g_fwd_missile_catched, g_fwd_casting_range, g_fwd_casting_outrange, g_fwd_npc_kills, g_fwd_npc_damage, g_fwd_initiation, g_fwd_gamestart, g_fwd_roundend, g_fwd_result, 
	...
	bool:g_npc_inattack[MAX_NPC], bool:g_valid_npc[MAX_NPC], g_npc_berserker[MAX_NPC], Float:g_npc_attack_delay[MAX_NPC], Float:g_class_data[MAX_CLASSES+1][MAX_DATA], bool:g_summoned_npc[MAX_NPC]
...
new g_Created_Npc = 0, g_To_Remove[MAX_NPC][2], g_Remover_Eng = 0, bool:g_disabled_Ent_Forwards, HamHook:g_entTrace, HamHook:g_entTakeDamage, HamHook:g_entThink, HamHook:g_entTouch,
	...
	Float:npc_custom_targets[MAX_NPC], Float:npc_custom_foes[MAX_NPC], npc_custom_animation[MAX_NPC], Float:npc_custom_move[MAX_NPC][3]
    
new Float:g_reach_point[MAX_NPC][3], Float:g_last_enemy_seek[MAX_NPC], Float:g_last_attack_time[MAX_NPC], Float:g_last_orig[MAX_NPC], Float:npc_hudinfo_delay[33], Float:g_Last_Good_Z[MAX_NPC]
...
#define max_way_points	50
new Float:g_way_point[MAX_NPC][3], npc_way_target[MAX_NPC], Float:g_way_cd[MAX_NPC], g_way_counter[MAX_NPC], g_way_point_index[MAX_NPC]
...
public plugin_init()
{
	...
	register_clcmd("say /stop",			"stop_testing"								);
	...
	if( containi(iMapName, "npc_") != -1 ) 
		b_Mode = true;
	if( containi(iMapName, "zc_") != -1  || equal(iMapName, "zp_boss_city")) 
		b_Mode = true;
}

public testing(id)
{
	for(new i = 0; i < g_way_counter[last_npc_created]; i++)
	{
		bomb_led(point[last_npc_created][i]) 
	}
	set_task(0.5, "testing", id)
}

public stop_testing(id)
	remove_task(id)
...
public Boss_ThinkHp(Hp)
{
	...
	pev(g_health_bar[Hp], pev_health, f_health)
	Procent = f_health * 100.0 / g_class_data[g_class_id[g_health_bar[Hp]]][DATA_HEALTH]
	set_pev(Hp, pev_frame, Procent)
	pev(g_health_bar[Hp], pev_origin, Origin)
...
public client_PreThink(id)
{
	/*if ( is_user_alive(id) )
	{
		if ( way_delay[id] <= get_gametime() )
		{
			new Float:vel[3];
			get_user_velocity(id, vel)
			if ( vector_length(vel ) > 180.0 )
			{
				if ( g_waypoints < g_max_waypoints )
				{
					pev(id, pev_origin, g_Ways[id][g_done_waypoints[id]++])
					way_delay[id] = get_gametime() + 0.7
					g_got_ways++
					g_waypoints++
				}
				else
...
public native_npc_take_aoe_damage(inflictor, Float:CreationOrigin[3], Float:Radius, Float:damage)
{
	static Float:fOrigin[3], Float:fDistance, Float:fTmpDmg;
	...
	for ( i=1; i<MAX_NPC; ++i )
	{
		if ( pev_valid(i) && !is_user_connected(i) )
		{
			if ( g_valid_npc[i] )
			{
				...
				fDistance = vector_distance(CreationOrigin, fOrigin);
				if(fDistance <= Radius)
				{
					if ( fDistance <= 300.0 )
						fTmpDmg = damage
					else
						fTmpDmg = damage - (damage / Radius) *2* fDistance;
...
public native_npc_set_move(ent, Float:fOrigin[3])
	npc_custom_move[ent] = fOrigin
...
stock collect_spawn(ent)
{
	new Float:origin[3];
	for (new i=1; i<=g_total_spawns*3; i++)
	{
		if ( g_boss[ent] )
			origin = g_spawns_boss[random(g_total_spawns_boss)]
		else
			origin = g_spawns[random(g_total_spawns)]
		if (check_spawn(origin)) 
		{
			entity_set_origin(ent, origin)//engfunc ( EngFunc_SetOrigin, ent, vf_NewOrigin );
		}
		return 1;
	}
...
{	
	...
	else if ( damage_type & DMG_SLASH || damage_type & DMG_BLAST )
	{}
	else
		return HAM_SUPERCEDE
		...
		switch( g_ent_events[ent] )
		{
			...
			case EVENT_CUSTOM:
			{
				...
				if ( npc_custom_move[ent][0] != CUSTOM_MOVE_OFF && npc_custom_move[ent][1] != CUSTOM_MOVE_OFF && npc_custom_move[ent][2] != CUSTOM_MOVE_OFF )
				{
					...
					pev(ent, pev_origin, Origin)
					npc_move(ent, Origin, npc_custom_move[ent])
				}
			}
			case EVENT_SHOOTER:
			{
...
stock NRS_Agression_Loop ( ent , distanced = 0 )
{
	...
	if ( distanced )
	{
		if(victim != 0 && can_see_fm(ent, victim))
		{
			if ( g_class_data[g_class_id[ent]][DATA_WEAPON_TYPE] > WEAPON_KNIFE )
			{
	...
	if(is_user_alive(victim))
	{
		if ( g_last_enemy_seek[ent] < get_gametime() ) 
		{
			...
			g_last_enemy_seek[ent] =get_gametime()+random_float(0.5, 1.0);
		}
		...
		if(distance <= g_class_data[g_class_id[ent]][DATA_ATTACK_DISTANCE])
		{
			...
			if  (g_npc_berserker[ent])
			{
				...
				ftime = g_class_data[g_class_id[ent]][DATA_ATTACK_WAITTIME];
			g_tv_sleep_time[ent] = get_gametime() + ftime
			entity_set_float(ent, EV_FL_nextthink, get_gametime() + g_last_attack_reloadtime[ent])
...
public NRS_delayed_damage( parm[3] )
{
	...
	pev(ent, pev_origin, Origin)
	pev(victim, pev_origin, VicOrigin)
	if(get_distance_f(Origin, VicOrigin) <= g_class_data[g_class_id[ent]][DATA_ATTACK_DISTANCE])
	{
...
{
	if ( !pev_valid ( witch ) || witch > MAX_NPC )
		return HAM_IGNORED;
...
public smoker_reelin(parm[]) // dragging player to npc
{
	...
	new Float:distance = get_distance_f(idOrigin, vicOrigin)
...
set_coord_velocity(ent, Float:angle[3], Float:Torigin[3], Float:got_origin[3]) // 1 = ent origin, 2 = target origin;
{
	...
	new Float:distance;
	distance = get_distance_f(Torigin, got_origin)
	...
	if (walk )
	{
		distance = get_distance_f(Torigin, g_Ent_Last_Origin[ent])
		
		if ( distance <= 5.0 && pev(ent,pev_flags)&FL_ONGROUND  && g_last_jump_anim[ent] < get_gametime())
		{
	...
	if ( walk || ( g_custom_npc[ent] == CUSTOM_HOTSEEK ) )   
	{
		set_pev(ent, pev_velocity, Direction)
	}
	...
	if ( g_custom_npc[ent] <= CUSTOM_OFF && g_last_orig[ent] < get_gametime() )
	{
		...
		g_last_orig[ent] = get_gametime()+0.5
	}
}
...
{
	NRS_create_npc(fOrigin,class_id)
}
...
public native_set_npc_boss(ent_id, bool:value)
{
	g_boss[ent_id] = value
	if ( value )
	{
		// Health bar
		new healthbar = engfunc(EngFunc_CreateNamedEntity, engfunc(EngFunc_AllocString, "info_target"))
		engfunc(EngFunc_SetModel, healthbar, BossSpriteHp)
		...
		set_pev(healthbar, pev_nextthink, get_gametime() + 1.0)
...
stock A_star_pathfinder(ent, Float:ent_origin[3], Float:End_Origin[3], Float:End_Way[][3])
{
	...
	while ( !finished || checked >= (max_way_points-1) )
	{
		...
		{	
			...
			// --| Free space found.
			/*engfunc ( EngFunc_TraceHull, i > 0 ? Got_Origin[i-1] : ent_origin, Got_Origin[i], DONT_IGNORE_MONSTERS, GetPlayerHullSize ( ent ), ent, 0 );
			if ( get_tr2 ( 0, TR_InOpen ) && !get_tr2 ( 0, TR_AllSolid ) && !get_tr2 ( 0, TR_StartSolid ) )
			{}else{
				cost[i]=10500;
				Broken_Point[i] = true
